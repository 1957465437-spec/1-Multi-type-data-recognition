...
### 一、 分类核心哲学
- **标准 (0)**：有完整的已知量、且逻辑闭合的问题或者具备明确的待证命题（如“试证明...”、“求证...”）以及必要的背景公理或前提条件的证明题。
- **脏 (1)**：逻辑中途夭折或是非题目体裁（如百科、纯代码块）。

### 二、 判定红线（出现以下情况必判为 1）
1. **[物理截断]**：逻辑链条断裂，语义中途断裂、公式残缺、选项不全。
   - **典型特征**：
     - 文本以逗号（，）、冒号（：）、连接词（且/但是）结尾，或最后一句明显没说完。
     - 引用外部图表、图片、表格但未提供关键数据（如“根据上表数据”、“如图所示”、“从上图可知”等，却未附表格/图片内容）。补充：“若题目为纯理论/公式推导题（如泛函分析、微分几何证明），且逻辑链条在文本层面已闭合，不视为缺失数据。”
...
### 三、 证明题专项放行准则
判定逻辑：凡包含“证明”、“求证”、“证明...”、“验证...”等引导词，且后续接续了完整的数学/物理陈述，即使文字极其精炼（如只有一行公式），只要逻辑自洽，即判定为 0。
符号容忍度：高等数学中的 LaTeX 符号（如 $\int, \nabla, \Omega$）不属于代码噪声，应视为正常自然语言的一部分。
